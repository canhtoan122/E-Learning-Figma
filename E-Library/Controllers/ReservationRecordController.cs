...
namespace E_Library.Controllers
{
    ...
    public class ReservationRecordController : ControllerBase
    {
        ...
        public ReservationRecordController(DataContext context)
        {
            _context = context;
        }

        [HttpGet]
        public async Task<ActionResult<List<ReservationRecord>>> Get()
        {
            return Ok(await _context.ReservationRecord.ToListAsync());
        }

        [HttpGet("search")]
        public async Task<ActionResult<IEnumerable<ReservationRecord>>> ReservationRecordSearch(string name)
        {
            try
            {
                IQueryable<ReservationRecord> query = _context.ReservationRecord;
                if (!string.IsNullOrEmpty(name))
                {
                    ...
                    query = query.Where(e => e.ReserveClass.Contains(name));
                    query = query.Where(e => e.ReserveDate.Contains(name));
                    query = query.Where(e => e.ReservePeriod.Contains(name));
                    query = query.Where(e => e.ReserveReason.Contains(name));
                }
                if (query.Any())
                {
                    return Ok(query);
        ...
        [HttpPost]
        public async Task<ActionResult<List<ReservationRecord>>> Add(ReservationRecord reservation)
        {
            _context.ReservationRecord.Add(reservation);
