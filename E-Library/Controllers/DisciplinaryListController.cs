...
namespace E_Library.Controllers
{
    ...
    public class DisciplinaryListController : ControllerBase
    {
        ...
        public DisciplinaryListController(DataContext context)
        {
            _context = context;
        }

        [HttpGet]
        public async Task<ActionResult<List<DisciplinaryList>>> Get()
        {
            return Ok(await _context.DisciplinaryList.ToListAsync());
        }
        [HttpGet("search")]
        public async Task<ActionResult<IEnumerable<DisciplinaryList>>> DisciplinaryListSearch(string name)
        {
            try
            {
                IQueryable<DisciplinaryList> query = _context.DisciplinaryList;
                if (!string.IsNullOrEmpty(name))
                {
                    ...
                    query = query.Where(e => e.DisciplinaryNumber.Contains(name));
                }
                if (query.Any())
                {
                    return Ok(query);
        ...
        [HttpPost]
        public async Task<ActionResult<List<DisciplinaryList>>> Add(DisciplinaryList disciplinary)
        {
            _context.DisciplinaryList.Add(disciplinary);
